...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < size; i++) {
		generated_pattern[i] = temp = get_pattern_value(size, i, temp, (argc > 1 ? atoi(argv[1]) : LINEAR_REV));
		printf("%c", temp);
	}

	/* Make sure only one thread does a writeDMA at the moment */
	for (i = 0; i < size; i++) {
		board->write(FPGA_ADDR + i, generated_pattern[i]);
		if (board->read(FPGA_ADDR + i) != generated_pattern[i]) {
			cerr << "PIO failed at offset " << (FPGA_ADDR + i) << endl;
			return 1;
		}
	}
}
