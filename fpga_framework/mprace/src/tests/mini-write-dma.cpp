 ...
 *
 */
static uint32_t get_pattern_value(unsigned int size, unsigned int offset,
...
//	DMABuffer buf(*board, (size * sizeof(int)), DMABuffer::KERNEL);

	/* Fill the DMA buffer with values */
	for (i = 0; i < size; i++) {
		buf[i] = generated_pattern[i] = temp = get_pattern_value(size, i, temp, (argc > 1 ? atoi(argv[1]) : LINEAR_REV));
		printf("%c", temp);
	}
